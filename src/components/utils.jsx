import {css, StyleSheet} from "aphrodite";import {
    Link as RouterLink
...
export const Container = ({children, className}) => {
    return (
        <div className={`mx-auto 2xl:max-w-[1320px] xl:max-w-[1140px] lg:max-w-[960px] md:max-w-[860px] px-4 lg:p-0 sm:max-w-[520px] w-full${className ? ' '+className : ''}`}>
            {children}
        </div>
    )
};
...
export const PageTitle = ({children}) => {
    return (
        <h3 className="text-2xl mb-3 text-gray-700">
            {children}
        </h3>
    );
};
...
export const Loader = ({size = null, color = null, className = ""}) => {
    ...
    return (
        <svg className={`animate-spin ${newColor} ${newSize} ${className}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
    );
};